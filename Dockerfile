...
RUN apt update && apt install -y --no-install-recommends expect bash fluxbox net-tools socat supervisor novnc x11vnc xvfb xterm firefox procps git mpv && apt clean && apt autoremove && rm -rf /var/lib/apt/lists/*
